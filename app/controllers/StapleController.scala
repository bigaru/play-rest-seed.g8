...
class StapleController(stapleRepo: StapleRepository, cc: ControllerComponents)(implicit ec: ExecutionContext) extends AbstractController(cc) {
  ...
  def createOne = Action.async(parse.json){ req =>
    val result = for (
      validStaple <- EitherT(validateBody(req.body));
      insertedStaple <- EitherT(stapleRepo.addOne(validStaple))
    ) yield insertedStaple
  ...
  def update(id: BSONObjectID) = Action.async(parse.json){ req =>
    val result = for (
      validStaple <- EitherT(validateBody(req.body));
      updatedStaple <- EitherT(stapleRepo.updateOne(id, validStaple))
    ) yield updatedStaple
