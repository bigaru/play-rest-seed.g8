  ...
  with MockitoSugar
{
  ...
  val insertedBook = Book("978-0425284629", "Skin in the Game", Some("Hidden Asymmetries in Daily Life"), "Nassim Nicholas Taleb", 304, Some(18.99))
  ...
  when(bookMongo.updateOne("978-0141031484".makeSelector, updatedBook.updateModifier)) thenReturn Future.successful(Some(updatedBook))

  when(bookMongo.updateOne("non-0141031484".makeSelector, Book("non-0141031484", "Fooled by Randomness", None, "Nassim Nicholas Taleb", 368, None).updateModifier)) thenReturn Future.successful(None)
